    ...
    addProductToCart(context, product) {
        ...
        if (product.inventory > 0) {
            ...
            if (!cartItem) {
                ...
                context.commit(CART.PUSH_PRODUCT_TO_CART, { id: product.id, canAdd: product.canAdd })
            } else {
                //如果有的话就增加items中对应数据的quantity数量就可以了
                context.commit(CART.INCREMENT_ITEM_QUANTITY, cartItem)
            }
            //跨modules调用products模块的减少产品的数量
            context.commit(`products/${PRODUCTS.DECREMENT_PRODUCT_INVENTORY}`, { id: product.id }, { root: true })
        }
    }
}
